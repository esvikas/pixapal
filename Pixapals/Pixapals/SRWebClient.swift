...
public class SRWebClient : NSObject
{
    ...
    public typealias FailureHandler = (AnyObject!, NSError!) -> Void
    ...
    */
    func build(dataDict:RequestData?) -> String? {
        ...
        if(dataDict != nil) {
            for (key, value):(String,AnyObject) in dataDict! {
    ...
    */
    public func data(image:[NSData], fieldName:[String], data:RequestData?) -> SRWebClient {
        if(image.count > 0 && self.urlRequest!.HTTPMethod == "POST") {
            ...
            if(data != nil && data!.count > 0) {
                postData += "--\(boundary)\r\n"
                    for (key, value):(String,AnyObject) in data! {
     ...
     public func send(success:SuccessHandler?, failure:FailureHandler?) -> SRWebClient {
        ...
        let blockOperation:NSBlockOperation = NSBlockOperation(block: {() -> Void in
            ...
            do {
                ...
                Ejson = try NSJSONSerialization.JSONObjectWithData(result!, options: [])
            ...
            NSOperationQueue.mainQueue().addOperationWithBlock({() -> Void in
                ...
                if (httpResponse != nil && httpResponse!.statusCode >= 200 && httpResponse!.statusCode <= 300) {
                    ...
                    if respHeaders[HeaderConstants.CONTENT_TYPE] == MimeConstants.APPLICATION_JSON {
                        ...
                        } catch let errorHere as NSError {
                            error = errorHere
                        } catch {
                            fatalError()
                        }
                    ...
                    failure!(Ejson, NSError(domain: self.urlRequest!.URL!.path!, code: httpResponse!.statusCode, userInfo: nil))
                } else if (failure != nil) {
                    failure!(Ejson, error)
                }
            })
